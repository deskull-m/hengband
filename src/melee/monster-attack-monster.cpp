...
void repeat_melee(PlayerType *player_ptr, mam_type *mam_ptr)
{
    ...
    for (auto &r : r_ptr->blow) {
        mam_ptr->effect = r.effect;
        mam_ptr->method = r.method;
        mam_ptr->d_dice = r.d_dice;
        mam_ptr->d_side = r.d_side;
        ...
        if (!mam_ptr->obvious && !mam_ptr->damage && (r_ptr->r_blows[r.id] <= 10))
            continue;

        if (r_ptr->r_blows[r.id] < MAX_UCHAR)
            r_ptr->r_blows[r.id]++;
    }
}
